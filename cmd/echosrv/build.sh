...
go build -ldflags "-X github.com/leaf-ai/platform-services/version.BuildTime=$DATE -X github.com/leaf-ai/platform-services/version.GitHash=$HASH" -o cmd/echosrv/bin/echosrv cmd/echosrv/*.go
go build -ldflags "-X github.com/leaf-ai/platform-services/version.BuildTime=$DATE -X github.com/leaf-ai/platform-services/version.GitHash=$HASH" -race -o cmd/echosrv/bin/echosrv-race cmd/echosrv/*.go
if ! [ -z "${TRAVIS_TAG}" ]; then
    if ! [ -z "${GITHUB_TOKEN}" ]; then
        github-release release --user leaf-ai --repo platform-services --tag ${TRAVIS_TAG} --pre-release && \
        github-release upload --user leaf-ai --repo platform-services  --tag ${TRAVIS_TAG} --name platform-services --file cmd/echosrv/bin/echosrv
    fi
fi
